...
int compress_db(int fd)
{
	...
	// Open temporary file for writing the compressed data
	tempFd = open(TMP_DB_FILE, O_RDWR | O_CREAT | O_TRUNC, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP);
	...
	// Read and copy valid records to temp file
	while (read(fd, &student, STUDENT_RECORD_SIZE) == STUDENT_RECORD_SIZE) {
		// Check if record is valid (not empty or deleted)
		if (memcmp(&student, &emptyRecord, STUDENT_RECORD_SIZE) != 0 && 
			student.id != DELETED_STUDENT_ID) {

			// Write valid record to temp file
			if (write(tempFd, &student, STUDENT_RECORD_SIZE) != STUDENT_RECORD_SIZE) {
				printf(M_ERR_DB_WRITE);
				close(tempFd);
				unlink(TMP_DB_FILE);
